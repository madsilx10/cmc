...
async def x_is_following(auth_token: str, ct0: str, user_id: str) -> bool:
    """Cek apakah akun ini sudah follow target via friendships/lookup."""
    headers = get_x_headers(auth_token, ct0)
    async with httpx.AsyncClient(timeout=30) as client:
        try:
            r = await client.get(
                f"https://x.com/i/api/1.1/friendships/lookup.json?user_id={user_id}",
                headers=headers,
            )
            if r.status_code == 200:
                data = r.json()
                if isinstance(data, list) and data:
                    conns = data[0].get("connections", [])
                    return "following" in conns
        except Exception:
            pass
    return False

async def x_is_retweeted(auth_token: str, ct0: str) -> bool:
    """Cek apakah akun ini sudah retweet tweet target."""
    headers = get_x_headers(auth_token, ct0)
    ...
    params = urlencode({"variables": variables, "features": features})
    url = f"https://x.com/i/api/graphql/VWFGPVAGkZMGRKGe3GFFnA/TweetDetail?{params}"
    async with httpx.AsyncClient(timeout=30) as client:
        try:
            r = await client.get(url, headers=headers)
            if r.status_code == 200:
                data = r.json()
                # cari tweet di result, cek retweeted_status atau legacy.retweeted
                entries = (
                    data.get("data", {})
                ...
                for inst in entries:
                    for entry in inst.get("entries", []):
                        ...
                        if legacy.get("id_str") == TWEET_ID and legacy.get("retweeted"):
                            return True
        except Exception:
            pass
    return False
...
async def x_follow(auth_token: str, ct0: str) -> bool:
    ...
    # cek sudah follow?
    if await x_is_following(auth_token, ct0, user_id):
...
async def x_repost(auth_token: str, ct0: str) -> bool:
    # cek sudah repost?
    if await x_is_retweeted(auth_token, ct0):
    ...
    payload = {
        "variables": {
            "tweet_id": TWEET_ID,  # string, bukan int
            "dark_request": False,
        },
