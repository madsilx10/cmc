...
async def x_follow(auth_token: str, ct0: str) -> bool:
    try:
        ...
        err = str(e)
        if "withheld_in_countries" in err:
            # Konten diblok di region ini, follow via user_id langsung
            try:
                client = await get_twitter_client(auth_token, ct0)
                users = await client.search_user(FOLLOW_TARGET)
                if users:
                    await users[0].follow()
                    print(f"  ✅ Follow @{FOLLOW_TARGET} berhasil (via search)")
                    return True
            except Exception as e2:
                print(f"  [X] Follow via search juga gagal: {e2}")
...
async def x_repost(auth_token: str, ct0: str) -> bool:
    try:
        ...
        if "already retweeted" in err.lower() or "327" in err:
            ...
            return True
        if "withheld_in_countries" in err:
            # Tweet diblok di region ini, coba retweet via create_tweet
            try:
                client = await get_twitter_client(auth_token, ct0)
                await client.create_retweet(TWEET_ID)
                print(f"  ✅ Repost tweet berhasil (via create_retweet)")
                return True
            except Exception as e2:
                print(f"  [X] Repost via create_retweet juga gagal: {e2}")
