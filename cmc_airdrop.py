...
def x_get_target_id(client, headers, username: str):
    """Lookup user_id via GraphQL internal Twitter"""
    ...
    r = client.get(
        "https://twitter.com/i/api/graphql/k5Ndt2-Y-1hbz2BEHRpxHw/UserByScreenName",
        headers=headers,
        params=params,
    )
    ...
    try:
        ...
        print(f"  [X] User ID tidak ditemukan di response")
        return None

def x_is_following(client, headers, username: str) -> bool:
    """Cek apakah sudah follow username via friendships/show"""
    r = client.get(
        "https://api.twitter.com/1.1/friendships/show.json",
        headers=headers,
        params={"target_screen_name": username},
    )
    if r.status_code != 200:
        return False
...
def x_follow(auth_token: str, ct0: str) -> bool:
    ...
    with httpx.Client(timeout=30) as client:
        # Cek dulu apakah sudah follow
        if x_is_following(client, headers, FOLLOW_TARGET):
            print(f"  ⏭️  Sudah follow @{FOLLOW_TARGET}, skip")
            return True

        target_id = x_get_target_id(client, headers, FOLLOW_TARGET)
        if not target_id:
            return False
        ...
        r = client.post(
            ...
            headers=headers,
            data={"user_id": target_id, "follow": "true"},
        )
...
def x_repost(auth_token: str, ct0: str) -> bool:
    ...
    with httpx.Client(timeout=30) as client:
        ...
        # 403 bisa berarti sudah retweet
        if r.status_code in (200, 403):
            print(f"  ✅ Repost tweet berhasil (atau sudah repost)")
            return True
...
def cmc_login(email: str, password: str) -> str | None:
    ...
    with httpx.Client(timeout=30) as client:
        ...
        if r.status_code == 200:
            ...
            # CMC minta verifikasi — coba ambil token dari response body dulu
            data = r.json()
            token = data.get("data", {}).get("token")
            if token:
                print(f"  ✅ Login CMC berhasil (202 dengan token)")
                return token
            # Kalau tidak ada token, print full response buat debug
            print(f"  ⚠️  CMC return 202 (butuh verifikasi atau IP flagged)")
            print(f"  [!] Response: {r.text[:300]}")
            return None

        print(f"  [X] Login CMC gagal: {r.status_code} {r.text[:300]}")
        return None
