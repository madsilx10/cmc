...
async def x_repost(auth_token: str, ct0: str) -> bool:
    ...
    try:
        async with AsyncSession(impersonate="chrome116") as client:
            ...
            # STEP 3: Mencoba Legacy Endpoint (Anti-404)
            ...
            if r.status_code == 200:
                print(f"  ✅ Repost asli tembus via Legacy API!")
                return True
                
            # STEP 4: Fallback ke GraphQL Cadangan jika Legacy gagal
            if r.status_code in [404, 400]:
                ...
                if r_alt.status_code == 200:
                    data = r_alt.json()
                    if data.get("data", {}).get("create_retweet") or "errors" in data:
                        print(f"  ✅ Repost asli tembus via GraphQL Alternatif!")
                        return True
                
                print(f"  [X] Repost gagal. Status Legacy: {r.status_code}, Status GraphQL: {r_alt.status_code}")
                return False
...
async def cmc_login(email: str, password: str) -> Optional[str]:
    payload = {"email": email, "platform": "web", "shaPassword
