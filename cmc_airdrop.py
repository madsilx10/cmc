...
# ── TWITTER MULTI-STAGE ENGINE (FIXED NOT TO 404) ───────────────────────────
...
async def x_repost(auth_token: str, ct0: str) -> bool:
    ...
    try:
        async with AsyncSession(impersonate="chrome116") as client:
            ...
            # STEP 3: Mencoba Tembak Endpoint Utama (GraphQL)
            repost_payload = {
                ...
                "queryId": "oB77v9wU6m2wXv7jI9Z7Sg",
            }

            r = await client.post(
                "https://x.com/i/api/graphql/oB77v9wU6m2wXv7jI9Z7Sg/CreateRetweet",
                headers=headers,
                json=repost_payload,
                timeout=15
            )
            ...
            if r.status_code == 200:
                ...
                for err in errors:
                    if err.get("code") == 327:
                        print(f"  命  Sudah repost tweet, skip")
                        return True
                    if err.get("code") == 344:
                ...
                if data.get("data", {}).get("create_retweet"):
                    print(f"  ✅ Repost asli tembus via GraphQL Utama!")
                    return True

            # STEP 4: Fallback ke Legacy Endpoint jika GraphQL utama 404/gagal
            print("  [*] Endpoint utama gagal/404, mencoba Legacy API...")
            ...
            if r_legacy.status_code == 200:
                print(f"  ✅ Repost berhasil via Legacy API!")
                return True
                
            # STEP 5: Fallback ke GraphQL Cadangan v2 jika Legacy juga gagal
            if r_legacy.status_code in [404, 400]:
                alt_payload = {
                    "variables": {"tweet_id": str(TWEET_ID), "dark_request": False},
                    "features": repost_payload["features"],
                    "queryId": "ojm99kID2btfN9wU6m2wXv",
                }
                
                r_alt = await client.post(
                    "https://x.com/i/api/graphql/ojm99kID2btfN9wU6m2wXv/CreateRetweet",
                    headers=headers,
                    json=alt_payload,
                    timeout=15
                )
                ...
                if r_alt.status_code == 200:
                    data_alt = r_alt.json()
                    if data_alt.get("data", {}).get("create_retweet") or "errors" in data_alt:
                        print(f"  ✅ Repost berhasil via GraphQL Alternatif!")
                        return True
                
                print(f"  [X] Repost gagal semua jalur. Status Utama: {r.status_code}, Legacy: {r_legacy.status_code}, Alternatif: {r_alt.status_code}")
                return False
                
            print(f"  [X] Repost gagal dengan HTTP Error {r.status_code}")
            return False
    except Exception as e:
