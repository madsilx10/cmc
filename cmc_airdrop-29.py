    ...
    async with httpx.AsyncClient(timeout=30) as client:
        try:
            ...
            if r.status_code == 202:
                print(f"  [X] CMC 202 — WAF/Cloudflare challenge. Body: {r.text[:300]}")
                return None

            if r.status_code == 400:
                print(f"  [X] CMC 400 — Bad request. Body: {r.text[:300]}")
                return None

            if r.status_code == 401:
                print(f"  [X] CMC 401 — Email/password salah. Body: {r.text[:300]}")
                return None

            if r.status_code == 403:
                print(f"  [X] CMC 403 — Forbidden/blocked. Body: {r.text[:300]}")
                return None
