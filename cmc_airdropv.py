...
async def x_get_user_id(auth_token: str, ct0: str, screen_name: str) -> Optional[str]:
    ...
    try:
        async with AsyncSession(impersonate="chrome122") as client:
...
async def x_repost(auth_token: str, ct0: str) -> bool:
    ...
    try:
        async with AsyncSession(impersonate="chrome122") as client:
            # STEP 1: Ping Main Home Page untuk menyeimbangkan Cookies Sesi internal X
            await client.get("https://x.com/home", headers=headers, timeout=15)
            await asyncio.sleep(1.5)

            # STEP 2: Tarik data Tweet Detail (Simulasi Membaca secara real)
            ...
            # Deteksi jika auth_token/ct0 mati total
            if r_view.status_code in [401, 403]:
                print("  [X] Gagal: Cookies Akun ini Invalid / Expired.")
                return False
