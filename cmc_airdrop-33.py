...
def load_waf_token() -> str:
    p = Path("waf.txt")
    if not p.exists():
        print("[!] waf.txt tidak ditemukan, CMC request mungkin kena block")
        return ""
    return p.read_text().strip()

# ── CMC ──────────────────────────────────────────────────────────────────────
WAF_TOKEN = ""  # di-load di main()
...
def cmc_headers(token: str = None) -> dict:
    h = {
        ...
        "cookie": f"aws-waf-token={WAF_TOKEN}" if WAF_TOKEN else "",
...
# ── MAIN ─────────────────────────────────────────────────────────────────────
async def run_account(idx: int, acc: dict, state: dict):
    email = acc["email"]
    print(f"\n{'='*50}")
