...
async def x_repost(auth_token: str, ct0: str) -> bool:
    ...
    # Perbaikan payload: TWEET_ID dipaksa string & penambahan parameter dark_request bawaan web
    payload = {
        "variables": {
            ...
            if r.status_code == 200:
                ...
                for err in errors:
                    ...
                    if "not found" in err.get("message", "").lower():
                        print(f"  [X] Tweet ID tidak ditemukan oleh Twitter. Cek TWEET_ID di config.")
                        return False

                # Must have create_retweet field dengan value (bukan null/empty)
                retweet_result = data.get("data", {}).get("create_retweet")
                if retweet_result:
                    print(f"  ✅ Repost tweet berhasil")
                    return True

                print(f"  [X] Repost gagal: create_retweet null. Response: {json.dumps(data)[:200]}")
                return False
            if r.status_code == 404:
                print(f"  [X] Repost 404 — Endpoint/Tweet ID gak valid. Gagal memproses payload.")
                return False
...
def cmc_headers(token: str = None) -> dict:
    ...
    if bearer:
        # normalize: hapus "Bearer" prefix dulu, tambah dengan spasi
