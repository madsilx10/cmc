...
async def x_repost(auth_token: str, ct0: str) -> bool:
    ...
    # HAPUS x-client-transaction-id acak karena memicu deteksi bot oleh X (Ghost Success)
    if "x-client-transaction-id" in headers:
        del headers["x-client-transaction-id"]

    # Payload wajib menyertakan 'features' agar terstruktur persis seperti browser asli
    payload = {
        "variables": {
    ...
    try:
        async with AsyncSession(impersonate="chrome116") as client:
            ...
            if r.status_code == 200:
                ...
                print(f"  [X] Repost gagal (ditolak X): {json.dumps(data)[:200]}")
                return False
