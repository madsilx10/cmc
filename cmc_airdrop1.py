...
# ── TWITTER (API GraphQL HTTP Client) ─────────────────────────────────────────
...
async def x_repost(auth_token: str, ct0: str) -> bool:
    ...
    try:
        async with AsyncSession(impersonate="chrome120") as client:
            print("  [*] Mensimulasikan pembacaan Tweet sebelum repost...")
            # ── SIMULASI MEMBUKA TWEET (GET) BIAR TIDAK DI-GHOSTING ──
            ...
            # Tarik detail tweet dulu agar session mencatat history logis manusia
            await client.get(f"https://x.com/i/api/graphql/064as96ZfCHYyvba0wY3dg/TweetResultByRestId?{view_params}", headers=headers, timeout=15)
            await asyncio.sleep(random.uniform(2.0, 3.5))

            # ── EKSEKUSI REPOST TWEET (POST) ──
            ...
            if r.status_code == 200:
                ...
                for err in errors:
                    if err.get("code") == 327:
                        ...
                        return True
                    if err.get("code") == 344:
                        print(f"  [X] Rate limit Twitter / Akun Terkunci Captcha.")
                        return False

                if data.get("data", {}).get("create_retweet"):
                    print(f"  ✅ Repost tweet beneran berhasil!")
                    return True

                print(f"  [X] Repost ditolak sistem: {json.dumps(data)[:150]}")
                return False
            print(f"  [X] Repost Error HTTP: {r.status_code}")
            return False
    except Exception as e:
        print(f"  [X] Request error atau Timeout: {e}")
        return False
