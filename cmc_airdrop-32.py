...
async def x_repost(auth_token: str, ct0: str) -> bool:
    ...
    async with httpx.AsyncClient(timeout=30, follow_redirects=False) as client:
        try:
            ...
            if r.status_code == 200:
                data = r.json()
                retweet_results = data.get("data", {}) or {}
                if retweet_results.get("create_retweet"):
                ...
                for err in errors:
                    if err.get("code") == 327:
                        ...
                        return True
                print(f"  [X] Repost gagal: {data}")
                return False
            print(f"  [X] Repost gagal: {r.status_code} {r.text[:300]}")
            return False
        except Exception as e:
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        try:
            ...
            if r.status_code == 200:
                ...
                if msg == "SUCCESS" or str(err) == "60005":
                    return True
